...
function vitePluginManusDebugCollector(): Plugin {
  return {
    ...
    transformIndexHtml(html) {
      ...
      const analyticsEndpoint = process.env.VITE_ANALYTICS_ENDPOINT?.replace(/\/$/, "");
...
const plugins = [
  ...
  compressionPlugin({
    ...
    threshold: 1024,
    verbose: false,
    disable: !isProductionBuild || process.env.DISABLE_PRECOMPRESSED_ASSETS === "true",
  }),
  compressionPlugin({
    algorithm: "gzip",
    ext: ".gz",
    ...
    rollupOptions: {
      output: {
        ...
        manualChunks: {
          ...
          "vendor-data": ["@tanstack/react-query", "superjson"],
        },
      },
    },
  },
